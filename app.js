...
(function(){
  ...
  function onScroll(){
    if (!ticking) {
      window.requestAnimationFrame(() => { parallax(); ticking = false; });
      ticking = true;
    }
  }
...
(function () {
  ...
  loadScript('https://unpkg.com/@studio-freight/lenis@1.0.38/dist/lenis.min.js')
    .then(() => {
...
(function(){
  ...
  window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(() => {
      ...
      viewport.scrollTo({ left: snapped });
  ...
  (function(){
    ...
    else window.addEventListener('load', initMarquee, { once: true });
